(ns foundrymfg.governor
  "Foundry Plant Operations Governor -- the independent compliance
  layer that earns the FoundryAdvisor the right to commit. The advisor
  ...
  tries to ACTUATE (rather than merely draft-schedule) the melting
  furnace or pouring line, whether a shipment proposal's own claimed
  ...
  direct furnace/pouring-line-equipment control, so this MUST be a
  separate system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:foundry-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    4. Furnace-actuate blocked     -- for `:schedule-maintenance`, does
                                       ...
                                       melting furnace or pouring line
    ...
    9. Tensile-test load
       ...
       tolerance                     -- for `:coordinate-shipment`
                                       (ADR-2607999800): INDEPENDENTLY
                                       recompute, FRESH every call,
                                       whether the batch's own recorded
                                       `:coupon-mass-kg` (a keel-block/
                                       Y-block test coupon poured
                                       alongside this heat/cast lot,
                                       ASTM A536/A216-style) yields a
                                       REAL `physics-2d`-simulated peak
                                       tensile load
                                       (`foundrymfg.robotics/run-
                                       tensile-test`, a genuine
                                       time-stepped rigid-body
                                       simulation) below the real
                                       disclosed minimum required
                                       tensile load
                                       (`foundrymfg.robotics/
                                       ...
                                       alloy-grade/defect-rate intake-
                                       patch fields by closed-set
   ...
   12. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [foundrymfg.registry :as registry]
...
(defn- furnace-control-blocked-violations
  ...
  Anything else (direct melting-furnace/pouring-line control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :furnace-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は溶解炉・注湯ライン等の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- furnace-actuate-blocked-violations
  ...
  to directly actuate the melting furnace or pouring line -- this
  actor may only ever propose/schedule a DRAFT maintenance window,
  never actuate the furnace/pouring line directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-furnace? (:value proposal))))
    [{:rule :furnace-actuate-blocked
      :detail "溶解炉・注湯ラインの直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- tensile-test-out-of-tolerance-violations
  "For `:coordinate-shipment` (ADR-2607999800): INDEPENDENTLY recompute
  -- FRESH, every call, never a previously stored/self-reported value
  -- whether the batch's own recorded `:coupon-mass-kg` (a keel-block/
  Y-block test coupon poured alongside this heat/cast lot, ASTM
  A536/A216-style separately-cast-test-coupon practice) yields a REAL
  `physics-2d`-simulated peak tensile load (`foundrymfg.robotics/run-
  tensile-test`, a genuine time-stepped rigid-body simulation) below
  the real disclosed minimum required tensile load (`foundrymfg.
  robotics/simulation-out-of-tolerance?`) -- never trusts any
  self-reported claim for the batch's own mechanical-property
  qualification. ADDITIONAL to every existing check, never replacing
  any of them (in particular `invalid-alloy-grade-violations`/
  `invalid-defect-rate-violations`, which validate the batch's own
  SELF-REPORTED intake-patch fields by closed-set membership/
  plausibility bound alone -- this check is the first ground-truth
  PHYSICS-derived mechanical-property check this actor performs). A
  batch with no `:coupon-mass-kg` on file (no tensile-test coupon
  poured/tested yet) never triggers this check -- missing telemetry is
  never silently treated as a violation, see `foundrymfg.robotics` ns
  docstring."
  [{:keys [op]} proposal st]
  (when (= op :coordinate-shipment)
    (let [batch-id (:batch-id (:value proposal))
...
(defn check
  "Censors a FoundryAdvisor proposal against the governor rules.
