(ns foundrymfg.sim
  "Demo driver -- `clojure -M:dev:run`. Walks a clean foundry through
  ...
  furnace or pouring line directly (permanently blocked, no
  override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated alloy grade, and a
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t1"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:alloy-grade :ductile-iron :last-assessed "2026-07-14"}}
                       coordinator))

    (println "== schedule-maintenance mnt-1 on furnace-001 (verified, registered, induction furnace -- escalates, approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on shakeout-002 (UNVERIFIED/unregistered shakeout line -> HARD hold) ==")
