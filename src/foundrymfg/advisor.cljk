(ns foundrymfg.advisor
  "FoundryAdvisor -- the *contained intelligence node* for the
  iron-and-steel casting foundry plant-operations coordination actor.

  It normalizes production-batch patches (alloy-grade/weight/defect-
  rate), drafts a furnace/mold/shakeout-equipment maintenance
  scheduling proposal against a piece of equipment, drafts a safety-
  concern flag, and drafts an outbound iron/steel casting shipment
  ...
  a real furnace/pouring-line actuation or freight dispatch. Every
  output is censored downstream by `foundrymfg.governor` before
  ...
  contract every caller of this actor agrees to) -- `foundrymfg.governor`
...
(defn- flag-safety-concern
  ...
  mold/core-binder fume exposure)/equipment-safety concern. ALWAYS
  ...
  `foundrymfg.phase`: no phase ever adds this op to a phase's `:auto`
  set; `foundrymfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound iron/steel casting shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed weight -- it does NOT invent one, and
  `foundrymfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは鋳造(鋳鉄・鋳鋼)フォンドリー工場プラント運用コーディネーターの"
       "助言者です。"
       ...
       "溶解炉・注湯ライン等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `foundrymfg.governor`
