(ns foundrymfg.operation
  "FoundryOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (FoundryAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Foundry Plant Operations
...
(defn build
  "Compiles a FoundryOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `foundrymfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FoundryAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Foundry Plant Operations Governor -- independent censor.
      (g/add-node :govern
