(ns foundrymfg.store
  "SSoT for the iron-and-steel casting foundry plant-operations
  ...
  iron-and-steel-casting-foundry capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch
                             (a poured heat/cast lot)'s alloy-grade/
                             ...
                             file in the foundry's production ledger;
                             `:shipped-weight-kg` tracks the batch's
                             own cumulative-shipped ground truth.
    - `equipment`         -- a melting-furnace/mold-station/shakeout-
    ...
    - `maintenance`       -- a scheduled furnace/mold/shakeout-
                             equipment maintenance-window DRAFT
                             against a piece of equipment
                             (`foundrymfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound iron/steel casting
                             shipment DRAFT (`foundrymfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (foundrymfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (foundrymfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :alloy-grade :ductile-iron :output-form :sand-cast
                :material "Ductile Iron (Nodular) Casting"
                :weight-kg 50000.0 :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-weight-kg 10000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :alloy-grade :cast-steel :output-form :sand-cast
                :material "Cast Steel Valve Body Castings"
                :weight-kg 8000.0 :defect-rate-percent 0.6
                :verified? true :registered? true
                :shipped-weight-kg 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :alloy-grade :gray-iron :output-form :sand-cast
                :material "Gray Iron Engine-Block Castings"
                :weight-kg 60000.0 :defect-rate-percent 2.1
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"furnace-001" {:id "furnace-001" :kind :induction-furnace
                  :verified? true :registered? true
                  :last-maintenance-date "2026-05-01"}
   "shakeout-002" {:id "shakeout-002" :kind :shakeout-line
                    :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `foundrymfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `foundrymfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered melting-furnace
  unit (schedulable for maintenance), one UNVERIFIED/unregistered
  shakeout-line unit (blocks any maintenance scheduling against it) --
