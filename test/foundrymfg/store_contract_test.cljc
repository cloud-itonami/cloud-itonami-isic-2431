(ns foundrymfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `foundrymfg.store` ns docstring for why a second (Datomic-backed)
...
(deftest sample-data-read-basics
  (let [s (seeded)]
    ...
    (is (false? (:verified? (store/equipment-unit s "shakeout-002"))))
    (is (false? (:registered? (store/equipment-unit s "shakeout-002"))))
    (is (= ["furnace-001" "shakeout-002"] (mapv :id (store/all-equipment s))))
    (is (= [] (store/ledger s)))
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:alloy-grade :gray-iron}})
    (is (= :gray-iron (:alloy-grade (store/batch s "batch-001"))))
...
(deftest safety-concern-flag-appends
  (let [s (seeded)]
    ...
    (store/commit-record! s {:effect :safety-concern/flag :path ["concern-2"]
                             :value {:equipment-id "shakeout-002" :severity :high}})
